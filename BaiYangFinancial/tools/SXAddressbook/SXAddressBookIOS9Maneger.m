...
- (NSArray *)getPersonInfoArray
{
    ...
    NSArray *keys = @[CNContactGivenNameKey, CNContactFamilyNameKey, CNContactPhoneNumbersKey,CNContactDatesKey,CNContactIdentifierKey];
    ...
    [contactStore enumerateContactsWithFetchRequest:request error:nil usingBlock:^(CNContact * _Nonnull contact, BOOL * _Nonnull stop) {
        ...
        personEntity.bookName = fullname;
        
        NSString * mobileId = contact.identifier;
        personEntity.mobileId = mobileId;
        
        NSArray * datesArr = contact.dates;
        NSString *dateStr = [NSString string];
        for (CNLabeledValue *labeledValue in datesArr) {
            NSDateComponents *dates = labeledValue.value;
            NSString * datesString = [NSString stringWithFormat:@"%ld-%ld-%ld",(long)dates.year,(long)dates.month,(long)dates.day];
            NSLog(@"%@",datesString);
            if (datesString.length > 0) {
                dateStr = [dateStr stringByAppendingString:datesString];
                dateStr = [dateStr stringByAppendingString:@","];
            }
        }
        if (dateStr.length > 1) {
            personEntity.dateStr = [dateStr substringToIndex:dateStr.length - 1];
        }
        ...
        if (fullPhoneStr.length > 1) {
            personEntity.mobile = [fullPhoneStr substringToIndex:fullPhoneStr.length - 1];
        }
...
- (void)contactPicker:(CNContactPickerViewController *)picker didSelectContactProperty:(CNContactProperty *)contactProperty
{
    ...
    [contactProperty.contact.phoneNumbers indexOfObjectPassingTest:^BOOL(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if([contactProperty.identifier isEqualToString:phoneObj.identifier]){
            ...
            [fullname replaceOccurrencesOfString:@"(null)" withString:@"" options:NSCaseInsensitiveSearch range:NSMakeRange(0, fullname.length)];
            personEntity.bookName = fullname;
            personEntity.mobile = phoneValue;
            self.chooseAction(personEntity);
            return true;
        }else{
            return false;

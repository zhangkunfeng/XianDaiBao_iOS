...
//        _dayLabel.center = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2 - 3);
       _dayLabel.layer.borderColor=[UIColor lightGrayColor].CGColor;
       _dayLabel.layer.borderWidth=0.3;
...
- (void)setupCollectionView {
    ...
    CGFloat itemHeight = DeviceHeight*0.0622;//itemWidth之前;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.dayLabel.textColor = [UIColor colorWithHex:@"333333"];
...
//    blackgroundView.backgroundColor = [UIColor colorWithHex:@"A9CDFD"];
//    cell.selectedBackgroundView = blackgroundView;
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        cell.dayLabel.textColor = [UIColor colorWithHex:@"999999"];
...
//        cell.chineseDayLabel.text = [self chineseCalendarOfDate:[self dateOfMonth:FDCalendarMonthNext WithDay:day]];
        ...
        if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date]) {
            cell.backgroundColor = [UIColor colorWithHex:@"A9CDFD"];
...
//        cell.chineseDayLabel.text = [self chineseCalendarOfDate:[self dateOfMonth:FDCalendarMonthCurrent WithDay:day]];
    ...
    //判断是否是本月
    if ([self compareOneDay:self.date withAnotherDay:[NSDate date]]==0) {
        ...
        //本月过去时间置灰色
        if ([self compareOneDay:selectedDate withAnotherDay:[NSDate date]]==-1) {
            cell.backgroundColor = [UIColor whiteColor];
            cell.dayLabel.textColor = [UIColor colorWithHex:@"999999"];
        }
    }
   //过去月份置灰色
    if ([self compareOneDay:self.date withAnotherDay:[NSDate date]]==-1) {
        cell.backgroundColor = [UIColor whiteColor];
        cell.dayLabel.textColor = [UIColor colorWithHex:@"999999"];

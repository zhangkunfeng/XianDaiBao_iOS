...
- (void)setupTitleBar {
    ...
    UIButton *leftButton = [[UIButton alloc] initWithFrame:CGRectMake(DeviceWidth*0.25, 0, 44, 44)];
    [leftButton setImage:[UIImage imageNamed:@"calendar_back"] forState:UIControlStateNormal];
    [leftButton addTarget:self action:@selector(setPreviousMonthDate) forControlEvents:UIControlEventTouchUpInside];
    [titleView addSubview:leftButton];
    
    UIButton *rightButton = [[UIButton alloc] initWithFrame:CGRectMake(titleView.frame.size.width-DeviceWidth*0.25-44, 0, 44, 44)];
    [rightButton setImage:[UIImage imageNamed:@"calendar_forward"] forState:UIControlStateNormal];
    ...
    titleButton.titleLabel.textColor = [UIColor colorWithHex:@"317df4"];//下面重新构成颜色
...
//    [titleButton addTarget:self action:@selector(showDatePicker) forControlEvents:UIControlEventTouchUpInside];
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 44, DeviceWidth, 0.5)];
...
- (void)setupWeekHeader {
    ...
    for (int i = 0; i < count; i++) {
        UILabel *weekdayLabel = [[UILabel alloc] initWithFrame:CGRectMake(offsetX, 44, (DeviceWidth - 30) / count, 46)];
        weekdayLabel.textAlignment = NSTextAlignmentCenter;
        weekdayLabel.text = Weekdays[i];
        
        if (i == 0 /*|| i == count - 1*/) {
            weekdayLabel.textColor =[UIColor colorWithHex:@"317df4"];
        } else {
            weekdayLabel.textColor = [UIColor colorWithHex:@"333333"];
        }
...
- (void)setupScrollView {
    ...
    [self.scrollView setFrame:CGRectMake(0, 90, DeviceWidth, self.centerCalendarItem.frame.size.height)];
...
- (void)setCurrentDate:(NSDate *)date {
    ...
    [self.titleButton setTitleColor:[UIColor colorWithHex:@"333333"] forState:UIControlStateNormal];
}
...
- (void)calendarItem:(FDCalendarItem *)item didSelectedDate:(NSDate *)date {
    ...
    NSLog(@"%ld年%ld月%ld日",(long)[comp year],(long)[comp month],(long)[comp day]);

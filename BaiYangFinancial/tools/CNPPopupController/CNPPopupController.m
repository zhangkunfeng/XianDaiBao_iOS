...
- (instancetype)initWithContents:(NSArray *)contents {
    ...
    if (self) {
        ...
        self.theme = [CNPPopupTheme defaultTheme:self.popupView.frame.size.width];
...
- (void)applyTheme {
    ...
    if (self.theme.popupStyle == CNPPopupStyleFullscreen) {
        ...
        maskBackgroundColor = self.theme.maskType == CNPPopupMaskTypeClear ? [UIColor clearColor] : [UIColor colorWithWhite:0.0 alpha:0.6];
    }
...
- (void)presentPopupControllerAnimated:(BOOL)flag {
    ...
    [UIView animateWithDuration:flag ? 0.5 : 0.0 animations:^{
...
- (CGPoint)endingPoint {
    ...
    } else if (self.theme.popupStyle == CNPPopupStyleCentered) {
        if (self.popupView.frame.size.width == 297) {
            if (iPhone4) {
                center = CGPointMake(self.maskView.center.x, self.maskView.center.y - (iPhoneHeight - 440) / 2 + 28);
            } else {
                center = CGPointMake(self.maskView.center.x, self.maskView.center.y - (iPhoneHeight - 500) / 2 + 28);
            }
        } else {
            if (iPhone5) {
                center = CGPointMake(self.maskView.center.x, self.maskView.center.y - 90);
            }else if (iPhone6) {
                center = CGPointMake(self.maskView.center.x, self.maskView.center.y);
            } else if (iPhone4) {
                center = CGPointMake(self.maskView.center.x, self.maskView.center.y - 60);
            } else {
...
+ (CNPPopupTheme *)defaultTheme:(CGFloat)maxWidth {
    ...
    defaultTheme.maxPopupWidth = maxWidth;
    return defaultTheme;
}

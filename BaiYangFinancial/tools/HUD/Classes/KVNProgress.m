...
static CGFloat const KVNContentViewWithoutStatusInset = 15.0f;
...
@property (nonatomic, weak) IBOutlet UIImageView *ErrorviewImageView;
...
//		_lineWidth = 2.0f;
        ...
        _statusFont = [UIFont systemFontOfSize:17.0f];
        //圆圈的颜色
        _circleStrokeForegroundColor = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:0.5];
        
        _circleStrokeBackgroundColor = [[UIColor darkGrayColor] colorWithAlphaComponent:0.5f];
        //圆圈里面的颜色
        _circleFillBackgroundColor = [UIColor clearColor];
        //提示框背景颜色 要设置为KVNProgressBackgroundTypeSolid类型
        _backgroundFillColor = [UIColor colorWithRed:240/255.0 green:239/255.0 blue:244/255.0 alpha:0.3];
...
//    CGFloat statusInset = (self.status.length > 0) ? KVNContentViewWithStatusInset : KVNContentViewWithoutStatusInset;
    ...
    CGFloat contentMargin = ([self isFullScreen]) ? KVNContentViewFullScreenModeLeadingAndTrailingSpaceConstraintConstant : iPhoneWidth * 0.25;
    ...
    if ([self isFullScreen]) {
        ...
        CGFloat contentWidth = CGRectGetWidth([UIScreen mainScreen].bounds) - (2 * iPhoneWidth * 0.8);
...
{
    self.ErrorviewImageView.hidden = YES;
...
//	[self animateError];
    self.ErrorviewImageView.hidden = NO;
}
...
//		self.contentView.image = backgroundImage;
        //修改背景颜色
        self.contentView.backgroundColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.5f];
	}
...
//	[self animateFullCircleWithColor:self.successColor];
    [self animateFullCircleWithColor:[UIColor whiteColor]];
...
- (CGFloat)circleSize
{
	...
	if (_circleSize == 0) {
		_circleSize = ([self isFullScreen]) ? 90.0f : 50.0f;
	}

...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
       /*NSLog(@"%f",self.frame.size.height);//278.00*/
...
- (void)setupNextAndLastMonthView {
    ...
    CGFloat itemH = self.frame.size.height / 9;
...
//    leftBtn.frame = CGRectMake(10, 10, 20, 20);
    leftBtn.frame = CGRectMake(itemW * 2 - 10, 0, itemW, itemH*1.5);
...
//    rightBtn.frame = CGRectMake(self.frame.size.width - 30, 10, 20, 20);
    rightBtn.frame = CGRectMake(itemW * 4 + 5, 0, itemW, itemH*1.5);
}

- (void)nextAndLastMonth:(UIButton *)button {
    if (button.tag == 1) {
        if (self.lastMonthBlock) {
            self.lastMonthBlock([self lastMonth:[NSDate date]]);
        }
    } else {
        if (self.nextMonthBlock) {
            self.nextMonthBlock([self nextMonth:[NSDate date]]);
        }
    }
}
...
//    CGFloat itemH     = self.frame.size.width / 7;
    ...
    self.headlabel.frame           = CGRectMake(0, 0, self.frame.size.width, itemH*1.5);
    ...
    lineLabel.frame = CGRectMake(10, CGRectGetMaxY(self.headlabel.frame) * 0.96, self.frame.size.width-20,0.5);
    lineLabel.backgroundColor = [UIColor lightGrayColor];
    [self.headlabel addSubview:lineLabel];
    ...
    self.weekBg.frame = CGRectMake(0, CGRectGetMaxY(self.headlabel.frame), iPhoneWidth, itemH*1.5);
    ...
    for (int i = 0; i < 7; i++) {
        ...
        week.frame    = CGRectMake(itemW * i, 0, itemW, itemH*1.5);
...
//    [dayBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
...
- (void)setStyle_Today:(UIButton *)btn
{
    ...
    solidLine.lineWidth = .5f ;
    solidLine.strokeColor = [UIColor colorWithHexString:@"#38C0C3"].CGColor;
    solidLine.fillColor = [UIColor clearColor].CGColor;
    CGPathAddEllipseInRect(solidPath, nil, CGRectMake(btn.frame.size.width/2-9,btn.frame.size.height/2-9,18 ,18));
    solidLine.path = solidPath;
    CGPathRelease(solidPath);
    [btn.layer addSublayer:solidLine];
    
//    btn.layer.cornerRadius = btn.frame.size.height / 2;
//    btn.layer.masksToBounds = YES;
    [btn setTitleColor:[UIColor colorWithHexString:@"#68CB00"] forState:UIControlStateNormal];
//    [btn setTitleColor:[UIColor redColor] forState:UIControlStateSelected];
...
//    [btn setTitleColor:[UIColor redColor] forState:UIControlStateSelected];
    [btn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
//    NSArray* array = [NSArray arrayWithObjects:@"13",@"15",@"28",@"31",@"8",@"19",@"1",nil];//测试数据
    for (NSString *str in self.allDaysArr) {
        if ([str isEqualToString:btn.titleLabel.text]) {
            UIImageView *stateView = [[UIImageView alloc] initWithFrame:CGRectMake(btn.frame.size.width/2, btn.frame.size.height-13, 13, 13)];
//            stateView.layer.cornerRadius = btn.frame.size.height / 2;
//            stateView.layer.masksToBounds = YES;
//            stateView.backgroundColor = self.allDaysColor;
//            stateView.image = self.allDaysImage;
//            stateView.alpha = 0.5;
            stateView.image = [UIImage imageNamed:@"勾选图标"];
            [btn addSubview:stateView];
        }
    }
...
- (UIColor *)allDaysColor {
    if (!_allDaysColor) {
        _allDaysColor = [UIColor lightGrayColor];
    }
